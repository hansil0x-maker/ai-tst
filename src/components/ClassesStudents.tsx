    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex border-b border-slate-700">
        <button 
          ...
          className={`flex-1 py-4 text-center font-bold text-lg transition-colors ${activeTab === 'classes' ? 'text-blue-400 border-b-2 border-blue-400 bg-slate-800/50' : 'text-slate-400 hover:text-slate-200'}`}
        >
          الصفوف
        </button>
        <button 
          onClick={() => setActiveTab('students')}
          className={`flex-1 py-4 text-center font-bold text-lg transition-colors ${activeTab === 'students' ? 'text-blue-400 border-b-2 border-blue-400 bg-slate-800/50' : 'text-slate-400 hover:text-slate-200'}`}
        >
          الطلاب
        </button>
      </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {filteredClasses.length > visibleClasses && (
            <div className="text-center pt-2">
              <button onClick={() => setVisibleClasses(v => v + 5)} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-full transition-colors border border-slate-700 text-sm shadow-lg">
                عرض المزيد
              </button>
            </div>
          )}
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-4">
            ...
            {filteredStudents.length > visibleStudents && (
              <div className="text-center pt-2">
                <button onClick={() => setVisibleStudents(v => v + 10)} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-full transition-colors border border-slate-700 text-sm shadow-lg">
                  عرض المزيد
                </button>
              </div>
            )}
